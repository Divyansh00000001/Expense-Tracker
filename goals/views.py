...
def add_goal(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            # Create goal object but don't save yet
            goal = form.save(commit=False)

            # Attach the logged-in user (owner)
            goal.owner = request.user

            # If your form has an "additional_amount" field:
            if 'additional_amount' in form.cleaned_data:
