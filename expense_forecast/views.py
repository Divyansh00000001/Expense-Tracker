...
def forecast(request):
    ...
    # Use groupby to calculate per-category totals, but guard against
    # pandas assertion errors on some environments by falling back to
    # a simple Python aggregation.
    try:
        category_forecasts = data.groupby('Category')['Expenses'].sum().to_dict()
    except AssertionError:
        # Fallback: iterate rows and sum amounts per category
        category_forecasts = {}
        for idx, row in data.reset_index().iterrows():
            cat = row.get('Category')
            amt = row.get('Expenses')
            try:
                amt = float(amt)
            except Exception:
                amt = 0.0
